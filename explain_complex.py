...
def explain_pharmacophore(pdb_id="4jwr"):
    ...
    # 6. Mapeamento Contínuo (Saliency Distribuído no Complexo)
    ...
    # Calcular percentis matemáticos para todos os nós (Receptor + Ligante)
    p99 = np.percentile(node_importance, 99)
    p95 = np.percentile(node_importance, 95)
    p85 = np.percentile(node_importance, 85)
    p70 = np.percentile(node_importance, 70)
    
    # Calcular arestas ultra-críticas (Top 1%) para manter as linhas tracejadas
    ...
    html_filename = f"pharmacophore_complex_{pdb_id}.html"
    with open(html_filename, "w", encoding="utf-8") as f:
        f.write(f'''<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <title>Complex Pharmacophore Extractor: {pdb_id}</title>
...
<body>
    <div id="info">
        <b style="color: #0f0; font-size: 1.2em;">PharmXAI-3D: Complex Pharmacophore</b><br><br>
        <b>Target:</b> {pdb_id}<br>
        <b>Afinidade Prevista:</b> {pkd_pred.item():.2f} pKd<br>
        <b>Afinidade Real:</b> {graph.y.item():.2f} pKd<br><br>
        <hr style="border-color: #444;">
        <b>FARMACOFORO DO COMPLEXO (RECEPTOR + LIGANTE):</b><br><br>
        <span style="color:magenta;">● Top 1% (Magenta):</span> Contatos Críticos Absolutos<br>
        <span style="color:orange;">● Top 5% (Laranja):</span> Ligações Fortes / Eletrostática<br>
        <span style="color:yellow;">● Top 15% (Amarelo):</span> Contornos Van der Waals<br>
        <span style="color:lime;">● Top 30% (Verde):</span> Influência Fraca do Solvente<br>
        <span style="color:magenta;">- Linhas Tracejadas:</span> Vetores Diretos (Top 1% Arestas)<br>
        <span style="color:cyan;">■ Ciano Base:</span> Arcabouço Tolerante a Mutações<br>
    </div>
...
''')
        ...
        # Render Nodes as Gradient Spheres + Elemental Cores (COMPLEX)
...
''')
    ...
    print(f"\n[SUCESSO] Viewer 3D do Complexo gerado: {html_filename}")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="Extract Complex Pharmacophore from a PDBbind Complex.")
